...
export default function LabourWelfareFundsAdmin() {
  ...
  useEffect(() => {
    ...
    const fetchExistingData = async () => {
      try {
        setLoading(true);
        const docRef = doc(db, "labourWelfareFunds", docId);
        ...
        if (snapshot.exists()) {
          ...
          setContributionType(data.contributionType || "Employer + Employee");
          setFrequency(data.frequency || "Half-Yearly");
          setNotes(data.notes || "");
          setHeaders(data.headers || []);
          setWages(data.wages || []);
          setPreview(true); 
        } else {
          alert("Requested Labour Welfare Fund entry configuration could not be found.");
        }
      } catch (err) {
  ...
  const publish = async () => {
    ...
    try {
      ...
      if (docId) {
        // Update operational record inside "labourWelfareFunds"
        await setDoc(doc(db, "labourWelfareFunds", docId), payload, { merge: true });
        alert("Labour Welfare Fund rules updated successfully!");
        navigate("/admin/labour-welfare-funds/manage"); 
      } else {
        // Direct record instantiation fallback
        await addDoc(collection(db, "labourWelfareFunds"), {
          ...payload,
          createdAt: new Date(),
        });
        alert("Labour Welfare Fund configuration saved successfully!");
      ...
      console.error(err);
      alert("Something went wrong while publishing data structural layout configuration.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen font-sans text-gray-900">
      ...
      {/* Dynamic Header Block with a Back Button */}
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
            {docId ? "Edit Labour Welfare Fund Rule" : "Labour Welfare Fund (LWF) Management"}
          </h2>
          <p className="text-gray-500 mt-1">
            {docId ? `Modifying targeted configuration rules matching identifier token: ${docId}` : "Configure state-wise statutory rules, frequencies, split contributions, and slabs templates."}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-7 gap-4 items-end mb-4">
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">
              {docId ? "Replace Slab CSV" : "Upload Slab CSV"}
            </label>
            <input
              type="file"
              accept=".xlsx,.xls,.csv"
      ...
      {preview && headers.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
          ...
          <div className="p-4 border-t border-gray-200 flex justify-end gap-3 bg-white">
            ...
            >
              <CheckCircle size={16} /> {docId ? "Update Rule Config" : "Save Data"}
            </button>
          </div>
        </div>
      )}
