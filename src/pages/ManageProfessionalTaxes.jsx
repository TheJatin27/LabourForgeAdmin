...
export default function ManageProfessionalTaxes() {
  const navigate = useNavigate();
  const [ptEntries, setPtEntries] = useState([]);
  const [searchTerm, setSearchTerm] = useState("");
  const [loading, setLoading] = useState(true);
  const [deletingId, setDeletingId] = useState(null);

  // FETCH ENTRIES FROM THE NEW FIRESTORE TARGET
  const fetchEntries = async () => {
    try {
      ...
      console.error("Error pulling professional tax datasets:", err);
      alert("Failed to load professional taxes datasets.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id, stateName) => {
    if (!window.confirm(`Are you sure you want to permanently delete the professional tax data for ${stateName}?`)) {
      return;
    }

    try {
      setDeletingId(id);
      await deleteDoc(doc(db, "professionalTaxes", id));
      alert(`${stateName} entry deleted successfully.`);
      // Update local state filter array directly to skip extra fetch network request
      setPtEntries((prev) => prev.filter((item) => item.id !== id));
    } catch (err) {
      console.error(err);
  ...
  // SEARCH FILTER LOGIC
  const filteredEntries = ptEntries.filter((entry) =>
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen font-sans text-slate-800">
      ...
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-bold text-slate-800">Manage Professional Taxes</h2>
          <p className="text-slate-500 mt-1">Review, track or delete published regional professional tax scale layouts.</p>
        </div>
      ...
      {filteredEntries.length === 0 ? (
        <div className="bg-white rounded-3xl border border-slate-200 p-16 text-center shadow-sm">
          ...
          <p className="text-slate-400 text-sm mt-1 max-w-sm mx-auto">
            {searchTerm ? "No matching records found for this search string filter." : "Get started by importing standard data structures from the Professional Taxes admin panel tab."}
          </p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-400 font-medium">
                      <span className="flex items-center gap-1">
                        ...
                        {entry.wages?.length || 0} Slab Categories Configured
                      </span>
                ...
                <div className="flex items-center justify-end gap-3 border-t border-slate-50 pt-4 md:pt-0 md:border-none">
                  ...
                  {/* EDIT BUTTON ROUTING INTEGRATION */}
                  <button
                    onClick={() => navigate(`/admin/professional-taxes/edit/${entry.id}`)}
                  ...
                  {entry.documentUrl ? (
                    <a
                      ...
                      className="flex items-center gap-2 border border-slate-200 text-slate-600 hover:text-blue-600 hover:border-blue-400 bg-white px-4 py-2.5 rounded-xl font-bold text-xs shadow-sm transition-all"
                    >
                      <ExternalLink size={14} /> View Gazette Notification
                    </a>
                  ) : (
