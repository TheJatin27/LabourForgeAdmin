...
import { useParams, useNavigate } from "react-router-dom"; // Hook to catch URL routing variables
...
export default function ProfessionalTaxesAdmin() {
  // Extracts the document ID parameter directly from route path: /edit/:id
  ...
  // Automatically fetches existing schema snapshot if docId exists in URL
  useEffect(() => {
    if (!docId) return;
    ...
    const fetchExistingData = async () => {
      try {
        setLoading(true);
        const docRef = doc(db, "professionalTaxes", docId);
        ...
        if (snapshot.exists()) {
          ...
          setStatus(data.status || "Applicable");
          ...
          alert("Requested Professional Tax entry configuration could not be found.");
        }
      } catch (err) {
        console.error("Error populating document values:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const publish = async () => {
    ...
    try {
      ...
      if (docId) {
        // Targeted mutation merge directly into the matching document reference path
        await setDoc(doc(db, "professionalTaxes", docId), payload, { merge: true });
        alert("Professional Tax layout updated and saved successfully!");
        navigate("/admin/professional-taxes/manage"); // Redirect back to list
      } else {
        // Fallback insertion block if accessing layout as direct creation field template
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen font-sans text-gray-900">
      ...
      {/* Dynamic Header Block with a Back Button to return to Manage layout safely */}
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
            {docId ? "Edit Professional Tax Configuration" : "Dynamic Professional Taxes Management"}
          </h2>
          <p className="text-gray-500 mt-1">
            {docId ? `Modifying targeted configuration rules matching unique token path identifier: ${docId}` : "Upload state slab layouts. Columns, fields, and headers adapt completely from uploaded template."}
          </p>
        </div>
        ...
        {docId && (
          <button
            onClick={() => navigate(-1)}
            className="inline-flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-md text-sm font-semibold text-gray-600 bg-white hover:bg-gray-50 transition-colors shadow-sm self-start sm:self-center"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4 items-end mb-4">
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">
              {docId ? "Replace Excel / CSV" : "Upload Excel / CSV"}
            </label>
            <input
              type="file"
              accept=".xlsx,.xls,.csv"
      ...
      {loading && (
        <div className="fixed inset-0 bg-white/60 backdrop-blur-[1px] flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl border border-gray-100 flex flex-col items-center gap-3">
            ...
            <p className="text-sm font-bold text-gray-700">Processing Structural Registry Changes...</p>
          </div>
        </div>
      )}
    </div>
